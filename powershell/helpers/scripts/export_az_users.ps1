# Split path
$Dir_Path = Split-Path -Parent "$env:dotfiles\powershell\scripts\*.*\"
...
$Csvfile = $Dir_Path + "\AllAzADUsers_$logDate.csv"

function export_az_users() {
	...
	@{Label = "Dirsync"; Expression = { if (($_.DirSyncEnabled -eq 'True') ) { 'True' } Else { 'False' } } }, @{Label = "Account status"; Expression = { if (($_.AccountEnabled -eq 'True') ) { 'Enabled' } Else { 'Disabled' } } } |
